...
#程序运行(runtime)时，TYPE_CHECKING永远都是False，类型检查器（type checker）会把它当成 True
#直接导入类会造成循环引用(circular import)，它们的作用不是为了运行，而是为了类型注解(type annotation)
#这样既避免了运行时的循环导入，又保留了完整的类型提示和 IDE 支持。
if TYPE_CHECKING:
